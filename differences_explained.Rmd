...
```{r breaking the correlation between t1 & IV, echo=F}
mu1 <- 5
mu2 <- 5
mu_WM <- 5
## variance
...
```{r plotting the broken correlation between t1 & IV, echo=F}
...
Word of warning: 
It is quite easily to inadvertently create data with measurement invariance. Since we would like to simulate 3-way correlated data, its a quite straight forward approach to make t2 a product of t1 (plus noise). Yet this would than have the hidden assumption that the reliability of the test in t1 and t2 are exactly the same at picking up the 'true signal' with another IV. Therefore, when you regress t1 on t2 (t2 ~ t1) of course all of the signal from the IV is lost (since the inherent 'true signal' is from the same source) and the issue with time invariant covariates in the residual-approach is hidden.

NOTE: I was accidently doing this with my rich-get-richer simulation?

```{r data with perfect true signal reliability, echo=T}
...
```{r adding differing correlations for the IV, echo=F}
...
multiple_ivcors <- ggplot(cor_bothT_with_WM_changingIVcor, aes(test_cor,prop_of_sig, col = type_of_test)) + geom_point() + geom_smooth() + theme_minimal() + scale_x_reverse() + labs(title = "How the residual method changes (with IV cor changing)")
...
COG_T2=~ .8*swm_between_errorsFU2+.9*rvp_aFU2+.7*prm_percent_correctFU2   # This specifies the measurement model for COG_T2  
...
# Intercepts
prm_percent_correctBL ~ 5*1
swm_between_errorsBL ~ 10*1
rvp_aBL ~ 33*1

prm_percent_correctFU2 ~ 5*1
swm_between_errorsFU2 ~ 10*1
rvp_aFU2 ~ 33*1
...
```{r definining the LCS model with strict measurement invariance}

LCS_forCFAsim <- '

WM_T1 =~ 1*swm_between_errorsBL + rvp_aBL + prm_percent_correctBL                           # This specifies the measurement model for WM_T1 
...
```{r function for parallel simulation, eval=F}

func_pvals <- function(model, s){
  library(lavaan)
  
  simdatCFA <- simulateData(cfa_strict_invariance, sample.nobs = s, meanstructure = T) #Simulate data
  fit_LCS_forCFAsim <- lavaan(LCS_forCFAsim, data = simdatCFA, estimator='mlr', fixed.x=FALSE, missing='fiml')
...
func_rmsea <- function(model, s){
  library(lavaan)
  simdatCFA <- simulateData(model, sample.nobs = s, meanstructure = T) #Simulate data
  fit_LCS_forCFAsim <- lavaan(LCS_forCFAsim, data = simdatCFA, estimator='mlr', fixed.x=FALSE, missing='fiml')
...
```{r RMSEA simulating data parallel, eval = F}
...
cfa_basic_rmsea <- foreach(i=1:1000, .combine=cbind) %dopar% {
   tempMatrix = func_rmsea(cfa_basic, 200) #calling a function
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
cfa_strong_rmsea <- foreach(i=1:1000, .combine=cbind) %dopar% {
   tempMatrix = func_rmsea(cfa_strong_invariance, 200) #calling a function
   #do other things if you want
...
cfa_strict_rmsea <- foreach(i=1:1000, .combine=cbind) %dopar% {
   tempMatrix = func_rmsea(cfa_strict_invariance, 200) #calling a function
   #do other things if you want
...
cfa_basic_rmsea <- as.data.frame(as.vector(cfa_basic_rmsea))
colnames(cfa_basic_rmsea) <- "rmsea"
cfa_strong_rmsea <- as.data.frame(as.vector(cfa_strong_rmsea))
colnames(cfa_strong_rmsea) <- "rmsea"
cfa_strict_rmsea <- as.data.frame(as.vector(cfa_strict_rmsea))
colnames(cfa_strict_rmsea) <- "rmsea"


rmsea_basic <- ggplot(cfa_basic_rmsea, aes(x=rmsea)) + geom_histogram() + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 

rmsea_strict <- ggplot(cfa_strong_rmsea, aes(x=rmsea)) + geom_histogram() + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 
...
```{r CFI simulating data parallel, eval = F}
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
cfa_basic_cfi <- foreach(i=1:1000, .combine=cbind) %dopar% {
   tempMatrix = func_cfi(cfa_basic, 200) #calling a function
   #do other things if you want
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
cfa_strong_cfi <- foreach(i=1:1000, .combine=cbind) %dopar% {
   tempMatrix = func_cfi(cfa_strong_invariance, 200) #calling a function
   #do other things if you want
...
cfa_strict_cfi <- foreach(i=1:1000, .combine=cbind) %dopar% {
   tempMatrix = func_cfi(cfa_strict_invariance, 200) #calling a function
   #do other things if you want
...
cfa_basic_cfi <- as.data.frame(as.vector(cfa_basic_cfi))
colnames(cfa_basic_cfi) <- "cfi"
cfa_strong_cfi <- as.data.frame(as.vector(cfa_strong_cfi))
colnames(cfa_strong_cfi) <- "cfi"
cfa_strict_cfi <- as.data.frame(as.vector(cfa_strict_cfi))
colnames(cfa_strict_cfi) <- "cfi"


cfi_basic <- ggplot(cfa_basic_cfi, aes(x=cfi)) + geom_histogram() + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 

cfi_strict <- ggplot(cfa_strong_cfi, aes(x=cfi)) + geom_histogram() + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 

cfi_strong <- ggplot(cfa_strict_cfi, aes(x=cfi)) + geom_histogram() + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 

```


```{r p-val on self-feedback covariance simulating data parallel, eval = F}
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
cfa_basic_pvals <- foreach(i=1:5000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_basic, 200) #calling a function
   #do other things if you want
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
cfa_strong_pvals <- foreach(i=1:5000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_strong_invariance, 200) #calling a function
   #do other things if you want
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
cfa_strict_pvals <- foreach(i=1:5000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_strict_invariance, 200) #calling a function
   #do other things if you want
...
cfa_basic_pvals <- as.data.frame(as.vector(cfa_basic_pvals))
colnames(cfa_basic_pvals) <- "pvals"
cfa_strong_pvals <- as.data.frame(as.vector(cfa_strong_pvals))
colnames(cfa_strong_pvals) <- "pvals"
cfa_strict_pvals <- as.data.frame(as.vector(cfa_strict_pvals))
colnames(cfa_strict_pvals) <- "pvals"


pvals_basic <- ggplot(cfa_basic_pvals, aes(x=pvals)) + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 

pvals_strict <- ggplot(cfa_strong_pvals, aes(x=pvals)) + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 

pvals_strong <- ggplot(cfa_strict_pvals, aes(x=pvals)) + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 
