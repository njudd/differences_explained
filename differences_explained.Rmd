...
- directionality on false-positives
- could ask him about the non-sensical simulation
...
cfa_strictLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_strict_invariance, LCS_strict, 100) #calling a function
   #do other things if you want
...
cfa_strongLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_strict_invariance, LCS_strong, 100) #calling a function
   #do other things if you want
...
cfa_weakLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_strict_invariance, LCS_weak, 100) #calling a function
   #do other things if you want
...
cfa_configuralLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals_configural(cfa_strict_invariance, LCS_configural, 100) #calling a function
   #do other things if you want
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
strongcfa_strictLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_weak_invariance, LCS_strict, 200) #calling a function
   #do other things if you want
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
strongcfa_strongLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_weak_invariance, LCS_strong, 200) #calling a function
   #do other things if you want
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
strongcfa_weakLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals(cfa_weak_invariance, LCS_weak, 200) #calling a function
   #do other things if you want
...
cl <- makeCluster(cores[1]-1) #not to overload your computer
registerDoParallel(cl)
strongcfa_configuralLCS_pvals <- foreach(i=1:10000, .combine=cbind) %dopar% {
   tempMatrix = func_pvals_configural(cfa_weak_invariance, LCS_configural, 200) #calling a function
   #do other things if you want
...
# the data is a list lets make it a df
strongcfa_strictLCS_pvals <- as.data.frame(as.vector(strongcfa_strictLCS_pvals))
colnames(strongcfa_strictLCS_pvals) <- "pvals"
strongcfa_strongLCS_pvals <- as.data.frame(as.vector(strongcfa_strongLCS_pvals))
colnames(strongcfa_strongLCS_pvals) <- "pvals"
strongcfa_weakLCS_pvals <- as.data.frame(as.vector(strongcfa_weakLCS_pvals))
colnames(strongcfa_weakLCS_pvals) <- "pvals"
strongcfa_configuralLCS_pvals <- as.data.frame(as.vector(strongcfa_configuralLCS_pvals))
colnames(strongcfa_configuralLCS_pvals) <- "pvals"

# lets put it in one df for plotting
df <- data.frame(type_invariance = c(rep("strict", dim(strongcfa_strictLCS_pvals)[1]), rep("strong", dim(strongcfa_strongLCS_pvals)[1]), rep("weak", dim(strongcfa_weakLCS_pvals)[1]), rep("configural", dim(strongcfa_configuralLCS_pvals)[1])),
                 pvals = c(strongcfa_strictLCS_pvals$pvals, strongcfa_strongLCS_pvals$pvals, strongcfa_weakLCS_pvals$pvals, strongcfa_configuralLCS_pvals$pvals))


pvals_strong <- ggplot(df, aes(x=pvals, color = type_invariance)) + theme_minimal() + geom_density(alpha=.2, fill="#FF6666") 
```
